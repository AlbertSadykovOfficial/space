...
let	drop_is_finished 	= false;
    ...
    function find_num(id)
    {
        elements = document.getElementsByClassName('js-cell');
        last_num_counter = 0;
        for (i = 0; i < elements.length; i++) 
        {
//					if (elements[i].children.length == 1) 
//					{
            last_num_counter += Number(elements[i].dataset.poslen);	
/*					}else
					{
						for (j=0; j < elements[i].children.length-1; j++)
						{
							last_num_counter += Number(elements[i].children[j].dataset.poslen);
						}
				}
*/
            if (elements[i].id == '_'+id)
            return last_num_counter;
        }
    ...
    {	
        if (document.getElementsByClassName('description_window').length > 0) 
        {
            close_description();
        }
        ...
        if (CHECK == 1) 
        {
            ...
            deadline_date = (x.deadline).substr(0,10);
            deadline_time = (x.deadline).substr(-5);
            out = 
            "<input type='text' name='task_id' 				value='"+param_2+"' style='display:none'	>"+
            "<button class='add_button' type='submit' name='change_task'>Изменить</button><br>"+
        ...
        {
            last_num = START_POSITION + find_num(param_1);  // project_main_paper.php
            ...
            deadline_date = '';
            deadline_time = '';
            out = "<input type='text' name='task_num' 				value='"+(Number(param_2)+1)+"' style='display:none'	>"+
                  "<input id='last_elem_num_input' type='text' name='last_elem_num' value='"+last_num+"' style='display:none'	>"+
                  "<button class='add_button' type='submit' name='create_task'>Create</button>"+
                "</form>";
        }
        id = param_1;
        output = "<div class='description_window'>"+"<div>"+
            "<form method = 'POST' action = 'project.php?project_id="+id+"' style='margin-top: 2%;'>"+
                "<input type='text' 						name='case_name' 				placeholder='case name' value='"+name+"'>"+
                "<button class='close_button' onclick='close_description()'><img src='"+server_content_folder+"/close.png'></button>"+
                "<span style='padding-left:5%;'>Executor:"+
                    "<input type='text'  						name='case_executor' 		placeholder='executor' 	value='"+executor+"'></span><br>"+
                "<span style='padding-left:5%;'>Deadline:"+
                    "<input type='date' name='deadline_date' value='"+deadline_date+"'>"+
                    "<input type='time' name='deadline_time' value='"+deadline_time+"'></span>"+
                "<span><p>Описание:</p>"+		
                "<textarea name='case_description' placeholder='description'>"+description+"</textarea></span>"+
            out +
            "</div></div>";
        document.getElementById('list').insertAdjacentHTML('afterend',output);
		}
