    <span class="project_switch_panel" onclick="switch_mode_num('left')" style="margin-top:20%;height:15%; z-index: 1000; border-top-right-radius:10px; border-bottom-right-radius:10px;"></span>
    <span class="project_switch_panel" onclick="switch_mode_num('right')" style="margin-left: 98%;margin-top:20%;height:15%; z-index: 1000; border-top-left-radius:10px; border-bottom-left-radius:10px;"></span>
<?php  
    ...
    if (isset($_POST['deadline_date'])) 
    {
        ...
        queryMySQL("UPDATE list SET deadline = '$deadline' WHERE id = '$id'");
    }
    ...
    if (isset($_POST['create_task'])) 
    {
        ...
        $last_number_of_this_list = (int)$_POST['last_elem_num'] + 1;

        if ($_POST['task_num'] == 1 && strlen($id) != 1) 
        {
            queryMySQL("UPDATE report SET id='$case_num', conclusion='<div>$case_name</div>' ,name='$case_name' WHERE id = '$id'");
        }
        else
        {	
            queryMySQL("UPDATE report SET position = position + 1 WHERE  position >= $last_number_of_this_list AND id LIKE '$project_id%'");
            queryMySQL("INSERT INTO report VALUES('$case_num',$last_number_of_this_list,'<div>$case_name</div>','$case_name')");
        }
		
        queryMySQL("INSERT INTO list VALUES('$case_num','$case_name','$case_description','$case_executor','$case_board','[]',NULL)");
    ...
    if (isset($_POST['change_task'])) 
    {
        ...
        queryMySQL("UPDATE report SET name='$case_name' WHERE id = '$case_id'");
    }
